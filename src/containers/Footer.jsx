  ...
  return (
    <div className='section pt-16 md:pt-20 pb-6 grid grid-cols-2 gap-x-6 gap-y-12 lg:grid-cols-3 text-light-gray relative'>
      ...
      <div className='w-full space-y-4'>
        ...
        <p className='max-w-[28ch] lg:max-w-[35ch] text-xs md:text-sm'>{getTranslation(language, 'footer.caption')}</p>
      </div>
      <div className='w-full space-y-4'>
        <h4 className='text-md font-semibold text-white'>{getTranslation(language, 'footer.contact')}</h4>
        <address className='text-xs md:text-sm space-y-2'>
          <p>
            Eibar, <br /> Gipuzkoa, <br /> Spain - 20600
          </p>
          <p>tactizity@gmail.com</p>
        </address>
      </div>
      <div className='w-full col-span-2 lg:col-span-1'>
        <p className='text-xs md:text-sm'>{getTranslation(language, 'footer.copyright')}</p>
      </div>
    </div>
  );
};
