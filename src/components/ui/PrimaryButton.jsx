...
const PrimaryButton = ({
  ...
  const baseStyles =
    'w-max px-[15px] py-2.5 rounded-full flex items-center gap-x-3 cursor-pointer';
  const variantStyles =
    variant === 'outlined'
  ...
  const handleMouseDown = () =>
    (StartIcon || EndIcon) && gsap.to('.icon', { scaleX: 1, duration: 0.2 });
  ...
  return (
    ...
    >
      ...
      {text && (
        <span className={`text-nowrap ${isResponsive && 'hidden lg:block'}`}>
          {text}
        </span>
      )}
