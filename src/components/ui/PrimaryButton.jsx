...
const PrimaryButton = ({text, onClick, startIcon: StartIcon, endIcon: EndIcon, isResponsive, variant, className}) => {
  const baseStyles = 'w-max px-[15px] py-2.5 rounded-full flex items-center gap-x-3 cursor-pointer';
  const variantStyles =
    variant === 'outlined'
  ...
  return (
    <button className={cn(`${baseStyles} ${variantStyles}`, className)} onClick={onClick} onMouseEnter={handleMouseUp} onMouseLeave={handleMouseDown}>
