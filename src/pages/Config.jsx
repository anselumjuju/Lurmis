		...
		<div className="w-full h-dvh flex flex-col lg:flex-row overflow-y-hidden select-none">
			...
			<div className={`flex-1 h-full ${!isFullScreen && 'max-h-[50vh] lg:max-h-full'} relative`}>
				<div className="w-full p-5 flex items-center absolute top-0 z-10">
					<PrimaryButton text={getTranslation('config.buttons.back')} startIcon={ArrowLeft} onClick={() => { setPage('home') }} isResponsive className={'lg:px-7 py-2.5'} variant={"outlined"} />
				...
				<div className="flex-1 max-w-screen-md h-full p-5 flex flex-col gap-y-3 overflow-y-auto z-10">
					<div className="flex items-center justify-between">
						...
						<div className="w-full flex justify-end gap-x-5 lg:hidden">
							...
							<PrimaryButton text={getTranslation('config.buttons.download')} startIcon={Download} onClick={takeScreenshot} isResponsive className={'lg:px-7 py-2.5'} variant={'outlined'} />
						</div>
					</div>
